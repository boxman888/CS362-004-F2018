...
int playCard(int handPos, int choice1, int choice2, int choice3,
  ...
  // play card
  if (cardEffect(card, choice1, choice2, choice3, state, handPos, &coin_bonus) <
      0) {
    return -1;
  }
...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  if (state->numBuys < 1) {
    ...
    gainCard(supplyPos, state, 0,
             who);  // card goes in discard, this might be wrong.. (2 means goes
                    // into hand, 0 goes into discard)

    state->coins = (state->coins) - (getCost(supplyPos));
    state->numBuys--;
    if (DEBUG)
      printf(
          "You bought card number %d for %d coins. You now have %d buys and %d "
          "coins.\n",
...
int drawCard(int player, struct gameState *state) {
  ...
  if (state->deckCount[player] <= 0) {  // Deck is empty
    ...
    // Shufffle the deck
    shuffle(player,
            state);  // Shuffle the deck up and make it so that we can draw
    ...
    state->hand[player][count] =
        state->deck[player][deckCounter - 1];  // Add card to the hand
    state->deckCount[player]--;
    state->handCount[player]++;  // Increment hand count
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3,
  ...
  switch (card) {
    ...
    case baron:
      ...
      if (choice1 > 0) {  // Boolean true or going to discard an estate
        ...
        while (card_not_discarded) {
          if (state->hand[currentPlayer][p] ==
              estate) {         // Found an estate card!
    ...
    case minion:
      ...
      else if (choice2)  // discard hand, redraw 4, other players with 5+ cards
    ...
    case tribute:
      ...
      if (tributeRevealedCards[0] ==
          tributeRevealedCards[1]) {  // If we have a duplicate card, just drop
                                      // one
    ...
    case sea_hag:
      for (i = 0; i < state->numPlayers; i++) {
        if (i != currentPlayer) {
          ...
          state->deck[i][state->deckCount[i]--] = curse;  // Top card now a
                                                          // curse
        }
      }
      return 0;
...
int discardCard(int handPos, int currentPlayer, struct gameState *state,
  ...
  if (handPos == (state->handCount[currentPlayer] -
                  1))  // last card in hand array is played
  {
