...
int AdventurerCard(struct gameState *state, int *currentPlayer, int *z, int *drawntreasure, int *cardDrawn, int temphand[])
{
	...
	*cardDrawn = state->hand[*currentPlayer][state->handCount[*currentPlayer]-1];//top card of hand is most recently drawn card.
	if (*cardDrawn == silver || *cardDrawn == gold)// Bug 3,  removed *cardDrawn == copper
	  *drawntreasure++;
	else{
...
{	
      //+3 Cards
      int i;
      for (i = 1; i < 3; i++)// Bug 2, i was 0 is now 1.
	{
	  drawCard(currentPlayer, state);
	}
...
int Council_RoomCard(int i, int currentPlayer, struct gameState *state, int handPos)
{
      //+4 Cards
      for (i = 0; i < 4; i++)
	{
      ...
      //+1 Buy 
      state->numBuys--;// Bug 1, numBuys was ++ now --.
...
int Embargo_card(struct gameState *state, int handPos, int currentPlayer, int choice1)
{
      //+2 Coins
      ...
      if ( state->supplyCount[choice1] == -1 )
	{
      ...
      state->embargoTokens[choice1]--; //Bug 4 removes emargo token, if no token, only bad stuff.
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
	    AdventurerCard(state, &currentPlayer, &z, &drawntreasure,&cardDrawn, temphand);

    case council_room:
	Council_RoomCard(i, currentPlayer, state, handPos);
    ...
    case embargo:
      Embargo_card(state, handPos, currentPlayer, choice1);
